...
public class TxtBookDAO implements BookDAO {
    private final static String booksFilePath = "src/resources/books.txt";
    ...
    @Override
    public void editBook(String title, String author, String editingAttribute, String newAttribute) throws DAOException {
        List<Book> bookList;
    ...
    @Override
    public List<Book> findByAttribute(String attribute, String searchingFilter) throws DAOException {
        List<Book> bookList;

        try {
            bookList = selectBooksByPredicate(scanBooksFromFile(), book ->
                    ((attribute.equals("TITLE") && searchingFilter.equals(book.getTitle()))
                            || (attribute.equals("AUTHOR") && searchingFilter.equals(book.getAuthor()))
                            || (attribute.equals("GENRE") && searchingFilter.toUpperCase().equals(book.getGenre().toString()))));
        } catch (Exception e) {
    ...
    private List<Book> scanBooksFromFile() throws IOException, NumberFormatException {
        List<Book> bookList = new ArrayList<>();

        try (Scanner scanner = new Scanner(Paths.get(booksFilePath))) {
            String[] bookAttributes;

            while (scanner.hasNext()) {
                bookAttributes = scanner.nextLine().split(attributeSeparator);
                Book temp = new Book();
    ...
    private void writeUsersToFile(List<Book> books) throws IOException {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(booksFilePath, false))) {
            writer.write("");
